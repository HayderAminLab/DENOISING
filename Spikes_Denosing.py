...
class Spikes_DENOISING_Function:
    ...
    def AnalyzeExp(self,expFile = None):
        """
        The denosing codes main function to filter the spikes based on the histogram and MBR of the spikes time. The main idea is to remove the random spikes, and keep the spikes with high frequency, the threshold is set based on the histogram and MBR of the spikes time. The results are saved in .npy file.
        Parameters
        ----------
        expFile: string
            the name of the file
        ...
        """
        if os.path.exists(self.srcfilepath + expFile[:-4] + '_denosed_SpikesChIDs' + '.npy') and os.path.exists(self.srcfilepath + expFile[:-4] + '_denosed_SpikesTimes' + '.npy'):
            SpikesChId_filter = np.load(self.srcfilepath + expFile[:-4] + '_denosed_SpikesChIDs' + '.npy')
            SpikesTimes_filter = np.load(self.srcfilepath + expFile[:-4] + '_denosed_SpikesTimes' + '.npy')
        else:
            ...
            np.save(self.srcfilepath + expFile[:-4] + '_denosed_SpikesChIDs', SpikesChId_last)
            np.save(self.srcfilepath + expFile[:-4] + '_denosed_SpikesTimes', SpikesTimes_last)
